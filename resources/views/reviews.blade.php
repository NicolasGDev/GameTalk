...
@section('content')
    <section class="max-w-[1280px] mx-auto  md:w-[90%] my-10">
        <div class="pb-4 px-4 md:px-0">
            <h2 class="text-white font-bold text-2xl pb-5">Nuestra selección de reseñas</h2>
        ...
        <div id="results" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            @foreach ($reviews as $review)
                <a href="{{ route('page.show', $review->id) }}" class="post-item">
                    <article
                        class="rounded-lg relative flex bg-center hover:border-2 border-main bg-no-repeat bg-cover items-end h-[300px] shadow-md overflow-hidden"
                        style="background-image: url('{{ $review->post_image }}')">
                        <div class="absolute inset-0 bg-black opacity-40"></div>
                        <div class="p-4 z-30">
                            <h3 class="text-2xl post-title text-white font-semibold mb-2">
                                {{ $review->title }}
                            </h3>
                            ...
                            <p class="text-white font-bold text-md">
                                {{ $review->created_at->locale('es')->diffForHumans() }}
                            </p>
                        </div>
                    </article>
                </a>
